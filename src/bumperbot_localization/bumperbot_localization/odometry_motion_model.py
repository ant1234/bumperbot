...
class OdometryMotionModel(Node):
    def __init__(self):
        ...
        if self.nr_samples >= 0:
            self.nr_samples = PoseArray()
            self.samples.poses = [Pose() for _ in range(self.nr_samples)]
        else:
            self.get_logger().fatal("Invalid number of samples : %d", self.nr_samples)
            return

        self.odom_sub_ = self.create_subscription(Odometry, "bumperbot_controller/odom", self.odomCallback, 10)
        self.pose_arry_pub_ = self.create_publisher(PoseArray, "odometry_motion_model/samples", 10)

    def odomCallback(self, odom):

        q = [odom.pose.pose.orientation.x, odom.pose.pose.orientation.x, 
        ...
        if sqrt(pow(odom_y_increment, 2) + pow(odom_x_increment)) < 0.01:
            delta_rot1 = 0.0
        else:
            delta_rot1 = angle_diff(atan2(odom_y_increment, odom_x_increment), yaw)

        delta_trasl = sqrt(pow(odom_y_increment, 2) + pow(odom_x_increment))
        ...
        for sample in self.samples.poses:
            ...
            delta_rot_1_draw = angle_diff(delta_rot1, rot1_noise)
            ...
            sample.position.x += delta_trans_draw * cos(sample_yaw + delta_rot_1_draw)
            sample.position.y += delta_trans_draw * sin(sample_yaw + delta_rot_1_draw)
            q = quaternion_from_euler(0.0, 0.0, sample_yaw + delta_rot_1_draw + delta_rot2_draw)
        ...
        self.pose_arry_pub_.publish(self.samples)
